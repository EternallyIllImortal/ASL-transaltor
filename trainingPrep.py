...
for label in labels:
    ...
    for file_name in os.listdir(label_dir):
        ...
        sequence_labels.append(label_map[label])
...
y = to_categorical(sequence_labels, num_classes=len(labels)).astype(int)

# Verify shapes
print(f"X shape: {X.shape}")  # Should be (num_samples, SEQUENCE_LENGTH, num_features)
...
model_save_path = os.path.join(BASE_DIR, 'asl_lstm_model_optimizedv3.keras')  # Save in the same directory
